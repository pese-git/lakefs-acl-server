...
## Итерация 1.1 — Подготовка проекта и инфраструктуры (1–2 дня) — ✅ выполнено

**Задачи:**

* Создать репозиторий проекта. — ✅
* Настроить виртуальное окружение / Docker. — ✅
* Добавить зависимости: FastAPI, SQLAlchemy, Alembic, Pydantic, psycopg2. — ✅ (Alembic не внедрён в prod, остальное реализовано)
* Настроить подключение к Postgres. — ✅ (но используется SQLite для dev, легко изменить)
* Создать базовую структуру проекта: — ✅ (структура улучшена, файлы вынесены по слоям)
...
**Выходной продукт:**
Рабочий skeleton проекта, FastAPI стартует и подключается к Postgres (или SQLite — реализовано)

---

## Итерация 1.2 — Базовая защита API (1–2 дня) — ✅ выполнено
...
## Итерация 1.3 — Модели данных и миграции (2–3 дня) — ✅ выполнено (кроме Alembic для миграций)
...
* Настроить Alembic для миграций. — ❌ (можно добавить позже, сейчас нет миграций)
* Провести первую миграцию — создать таблицы в БД. — ✅ (create_all из кода)
...
## Итерация 1.4 — CRUD Users API (2–3 дня) — ✅ выполнено

* Endpoint’ы: `POST /auth/users`, `GET /auth/users`, `GET /auth/users/{user_id}`, `DELETE /auth/users/{user_id}` — ✅
* Все endpoint’ы защищены токеном (`TokenAuthMiddleware`). — ✅
...
## Итерация 1.6 — CRUD Policies API (2–3 дня) — ✅ выполнено
